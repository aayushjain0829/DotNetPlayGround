...
internal class Solution1718 : Solution<int, CustomEnumerable<int>>
{
    #region Algos
    private int[] ConstructDistancedSequenceCheat(int n)
    {
        int[][] solns = [[0],
                        [1],
                        [2, 1, 2],
    ...
    private int[] ConstructDistancedSequence(int n)
    {
        if (n == 1) return [1];
        ...
        bool Construct()
        {
            ...
            if (idxToFill == -1) return true;

            for (int i = n; i > 0; i--)
            {
                if (vis[i]) continue;
                if (i != 1 && (idxToFill + i >= 2 * n - 1 || ans[idxToFill + i] != 0)) continue;

                ans[idxToFill] = i;
                if (i != 1) ans[idxToFill + i] = i;
                vis[i] = true;
                if (Construct()) return true;
                vis[i] = false;
                if (i != 1) ans[idxToFill + i] = 0;
                ans[idxToFill] = 0;
            }
    ...
    public override CustomEnumerable<int> Execute(int input)
    {
        var cheatAns = ConstructDistancedSequenceCheat(input);
        var AlgoAns = ConstructDistancedSequence(input);
        bool IsAlgoAnsCorrect = cheatAns.SequenceEqual(AlgoAns);

        Console.WriteLine($"Test Case {input}: {(IsAlgoAnsCorrect ? "Passed" : "Failed")}");
        return new(ConstructDistancedSequence(input));
    }
